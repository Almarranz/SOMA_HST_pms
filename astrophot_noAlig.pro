    ...
    For flt = 0, N_ELEMENTS(filters)-1 DO BEGIN
        ...
        FOR i = 0, N_ELEMENTS(epochs)-1 DO BEGIN
            ...
            ;   psf = readfits(path + 'psf_'+filter+'.fits') ;use Rainer's PSF worst case scenerio. psf = readfits(path + 'psf_'+filter+'_rainer.fits')
              psf = readfits(tmpdir + 'psf_gui_'+filter+'.fits') ;use Rainer's PSF worst case scenerio. psf = readfits(path + 'psf_'+filter+'_rainer.fits')
            ...
            ;   im = readfits(path + nam + '.fits',header,EXT=0) ;careful here! check the extension!
              ...
              im[isnan] = 0
              noise = sqrt(im)
            ;   noise = READFITS(path + nam + '.fits',header, EXT=2)

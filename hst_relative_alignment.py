...
# for zone in zones:
    ...
    for band in band_ls:
        ...
        for epoch in range(1,3):
            ...
            # if red_techn == 'Gaia':
            #     results = f'/Users/amartinez/Desktop/Projects/SOMA_HST_pm/sf/results/{zone}_Align/f{band}/epoch{epoch}/'
        ...
        ax.set_title(f'{zone} {b_name}')
        ax.scatter(cat1['ra'], cat1['dec'], label = 'Ep1' )
        ax.scatter(cat2['ra'], cat2['dec'], label = 'Ep2')
        ...
        diff_H = cat2_match[f'{b_name}'] - cat1_match[f'{b_name}']
        ...
        h = ax.hexbin(cat1_match[f'{b_name}'] , diff_H, norm = LogNorm())
        cbar = plt.colorbar(h, ax=ax)
        # ax.scatter(cat1_match[f'{b_name}'] , diff_H)
        ax.legend()
        ax.set_ylabel(f'$\Delta {b_name}$', fontsize = 12)
        ax.set_xlabel(f'[{b_name}]', fontsize = 12)
        
        var_m = (abs(diff_H) > 4) & (cat1_match[f'{b_name}'] < 20) 
        
        
        
        region(cat1_match[var_m], 'ra', 'dec',
                name = f'Var_{zone}_Ep1__Band{b_name}_stars_xy',
                save_in = pruebas,
                wcs = 'fk5',
                color = 'blue',
                marker = 'x')
        region(cat2_match[var_m], 'ra', 'dec',
                name = f'Var_{zone}_Ep2__Band{b_name}_stars_xy',
                save_in = pruebas,
                wcs = 'fk5',
                color = 'cyan',
                marker = 'x')
        ...
        ax.set_xlim(-1,1)
        ax2.hexbin(cat2_match[f'{b_name}'],diff_H[np.logical_not(mask_H.mask)], norm = LogNorm())
        ax2.set_ylim(-1,1)
        ...
        if destination == 1:
            ...
            cat2 = alg_loop(cat2, cat1, 'xp', 'yp', align_by, max_deg, d_fa.to(u.arcsec).value, max_loop,sig_cl_H = sig_cl_H_aligment, 
                            grid_s = grid_s, grid_Hmin = grid_Hmin, grid_Hmax = grid_Hmax ,isolation_radius = isolation_radius,  f_mode = f_mode, mag_name = b_name, mag_lim_alig= None)
        ...
        cat1_mi.write(pm_results + f'pm_{zone}_{band}_posEpo1.txt', format = 'ascii', overwrite = True)
